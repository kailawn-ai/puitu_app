...
export default function CommunityChatBubble({
  ...
  return (
    <View className={`mb-3 ${isOwn ? "items-end" : "items-start"}`}>
      ...
      >
        {!isOwn && !!senderName ? (
          <Text className="mb-1 text-xs font-semibold text-primary-500">
            {senderName}
          </Text>
        ) : null}
        ...
        <View className="mt-2 flex-row items-center justify-end">
          ...
          <Text
            className={`text-[11px] ${
              isOwn ? "mr-1 text-white/80" : "text-slate-400 dark:text-slate-500"
            }`}
          >
            {createdAtLabel}
          </Text>
          {isOwn ? <CheckCheck size={12} color="rgba(255,255,255,0.85)" /> : null}
        </View>
      </View>
    </View>
  );
